...
import { intercompanyCells, withoutIntercompany } from './intercompanyService';
...
export function consolidatedValues(
  ...
  onlyEntities?: string[],
  /**
   * Leave the group's own settlements out, so the total is what the countries
   * expect to move with the world outside it. See `withoutIntercompany`.
   */
  ...
  for (const e of all) {
    ...
    const figures = excludeIntercompany ? withoutIntercompany(sub, template) : sub.values;
