...
| **Questions** (every role) | The board of every question **treasury and the approvers** have asked — treasury sees the group, everyone else the entities they are responsible for. Three **collapsible columns**, left to right: **awaiting a reply**, **answered**, **closed**, one card per conversation, carrying the country, week, line item, the figures behind it, **who asked and in what capacity**, and — for a thread with history — **the last question and the answer to it**, with the rest counted above. Treasury's columns group into **regions, closed by default** with the countries named on the band, so eleven countries fit on one screen; a search opens them. Filters for who asked (**Both / Treasury / Approvers**, with live counts), region, period and a search across the threads. Opening a card shows the **whole thread as a conversation** with a reply box — the submitter answers, the asker follows up, and the card moves columns as it goes — plus **Open Forecast** onto the cell and **Mark Reviewed** to close it off |
...
person waiting. The capacity is resolved from the **user directory** whenever
the name is known there, not from whatever the stored message claims: older
versions stamped every question "treasury", so an approver's own question came
back to the submitter attributed to Treasury, and a stored role that
contradicts who somebody is is simply wrong.
